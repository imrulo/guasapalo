...
export function Header() {
  return (
    <header className="sticky top-0 z-30 border-b border-line/90 bg-night/85 backdrop-blur-md">
      <div className="mx-auto flex min-h-14 max-w-5xl items-center justify-between gap-3 px-5 sm:min-h-16 sm:px-8">
        <Link
          href="/"
        ...
        <nav aria-label="Principal" className="flex items-center gap-1 sm:gap-2">
          {nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="inline-flex min-h-11 items-center px-2.5 text-sm text-muted transition-colors duration-200 hover:text-cream sm:px-3"
            >
              {item.label}
            </Link>
          ))}
          <WhatsAppCta
            ...
            className="ml-1 min-h-11 px-3 text-sm sm:px-4"
          >
            WhatsApp
          </WhatsAppCta>
        </nav>
